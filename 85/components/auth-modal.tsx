...
const TikTokIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" shapeRendering="geometricPrecision" textRendering="geometricPrecision" imageRendering="optimizeQuality" fillRule="evenodd" clipRule="evenodd" viewBox="0 0 461 512.235" width="16" height="16">
    <g fillRule="nonzero">
...
export function AuthModal({ isOpen, onClose, onSuccess }: { isOpen: boolean, onClose: () => void, onSuccess: (user: any) => void }) {
  ...
  const handleDiscordLogin = async () => {
    try {
      // Open window synchronously to avoid popup blockers
      const authWindow = window.open('', 'oauth_popup_discord', 'width=600,height=700');
      if (!authWindow) {
        alert('Please allow popups for this site to connect your account.');
        return;
      }
          ...
          <div className="p-6 flex flex-col gap-3">
            {view === 'options' ? (
              <>
                ...
                <button onClick={handleDiscordLogin} className="w-full bg-[#1c1c1c] hover:bg-[#2a2a2a] py-3.5 px-4 rounded-full text-zinc-200 text-[15px] font-medium transition-colors flex items-center justify-center gap-2 border-none">
